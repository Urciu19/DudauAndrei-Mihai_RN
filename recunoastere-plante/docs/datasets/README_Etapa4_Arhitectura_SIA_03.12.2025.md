...
Etapa 4 corespunde cerinței:
„Dezvoltarea arhitecturii aplicației software bazată pe RN”

În această etapă NU se cere un model antrenat bine, ci un schelet funcțional al întregii aplicații, cu toate modulele integrate:

✔ Ce trebuie să funcționeze:

Preprocesarea completă a imaginilor (pipeline real implementat)

Structurarea datasetului (train/val/test)

Definirea și compilarea rețelei neuronale (fără antrenare)

Aplicarea pipeline-ului end-to-end

O interfață/UI sau endpoint care primește imagine și returnează predicție dummy

Arhitectura completă: Data Acquisition → Preprocessing → Neural Network → UI

✘ Ce NU trebuie acum:

Acuratețe mare

Model final antrenat
...
| --------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------- |
| Identificarea rapidă a speciilor de plante în aplicații educaționale sau de teren | Imaginea este procesată → fundal eliminat → CNN clasifică specia → sistemul returnează numele comun, numele latin și o descriere | Preprocessing Module + Neural Network Module + UI Module |
| Standardizarea datelor vizuale pentru antrenarea unui model robust                | Eliminarea background-ului și redimensionare uniformă la 150×150 px pentru toate imaginile                                       | Preprocessing Pipeline (src/preprocessing/)              |
| Extinderea datasetului cu imagini noi, sintetice sau generate                     | Generarea imaginilor cu AI → preprocesarea lor → includere în dataset                                                            | Data Acquisition Module (src/data_acquisition/)          |
